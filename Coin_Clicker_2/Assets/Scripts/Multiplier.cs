...
public class Multiplier : MonoBehaviour
{
    ...
    public double Cost
    {
        get
        {
            double baseCost = UpgradeHandler.IsUpgradePurchased(3, 0) ? 1.5 : 2;
            return 1000
    ...
    public void UpdateDisplays()
    {
        ...
        multiplierDisplay.text = NumberFormatter.FormatNumber(CoinMulti, 2) + "x coins";
        if (UpgradeHandler.IsUpgradePurchased(1, 3))
            multiplierDisplay.text += "\n" + NumberFormatter.FormatNumber(UpgradeHandler.GetEffectOfUpgrade(1, 3), 2) + "x clickpoints";
